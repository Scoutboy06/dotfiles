...
Item {
  ...
  // The omarchy-shell host injects these after dynamically loading a custom bar.
  property string omarchyPath: Quickshell.env("OMARCHY_PATH") || "/usr/share/omarchy"
  property var barWidgetRegistry: null
  // Holds the `bar:` subtree: position, centerAnchor, layout. The host owns file
  // IO; the bar just renders whatever it is handed. The bar font follows the
  ...
  function applyBarConfig() {
    ...
    islandMode = config.islands === true
  ...
  component BarPanel: PanelWindow {
    ...
    color: root.islandsEnabled || root.transparent ? "transparent" : root.background
  ...
  component HorizontalSectionIsland: Item {
    id: sectionIsland

    required property var entries
    required property string region
    property int leadingPadding: 4
    ...
    ModuleList {
      id: sectionModules
      entries: sectionIsland.entries
      region: sectionIsland.region
      anchors.left: parent.left
